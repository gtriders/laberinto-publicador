(() => {
  ...
  window.addEventListener('laberinto:studio-media', async (event) => {
    ...
    try{
      ...
      const source=await r.blob(),blob=await toFourFive(source),file=new File([blob],`estudio-${d.frame||'ia'}-4x5.png`,{type:'image/png'}),dt=new DataTransfer();dt.items.add(file);
