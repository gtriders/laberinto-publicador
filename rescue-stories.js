(() => {
  ...
  async function scheduleStory(now=false){if(!current.imported)return;let dt=$('#igStorySchedule').value;if(now){const d=new Date(Date.now()-30000);dt=localValue(d);$('#igStorySchedule').value=dt;}const scheduled=new Date(dt);if(!dt||Number.isNaN(scheduled.getTime())){status('Elige una fecha y hora válidas.');return;}if(!now&&scheduled.getTime()<=Date.now()){status('La fecha debe estar en el futuro.');return;}const text=$('#igStoryText').value.trim(),when=now?'ahora':`para ${scheduled.toLocaleString('es-CL')}`;if(!confirm(`¿Enviar esta historia ${when}? Revisa antes el texto sobre la imagen.`))return;const buttons=[$('#igStoryNow'),$('#igStoryScheduleBtn')];buttons.forEach(b=>b.disabled=true);status('Creando formato 9:16…');try{const blob=await renderStoryBlob(current.imported.media_url,text,brands[current.brandId]||current.brandId);status('Guardando la historia…');const uploaded=await uploadBlob(blob,current.brandId);status(now?'Enviando a la cola para publicar ahora…':'Programando la historia…');await call({action:'schedule',brand_id:current.brandId,title:`Historia · Rescate`,caption:text||'Historia',media_url:uploaded.media_url,media_path:uploaded.media_path,media_type:'STORIES',ai_analysis:{...(current.analysis||{}),rescue_source:'instagram',content_type:'story',original_media_id:current.mediaId,story_overlay:text},scheduled_at:scheduled.toISOString()});document.querySelector('#aiRefreshQueue')?.click();status(now?'Historia enviada. El publicador automático la tomará en el próximo ciclo.':'Historia programada.');setTimeout(()=>dialog.close(),900);}catch(e){status((e.message||'No se pudo programar la historia')+(e.detail?' · '+e.detail:''));}finally{buttons.forEach(b=>b.disabled=false);}}
