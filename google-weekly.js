  ...
  function boot(){
    ...
    function bind(root){
      root.querySelectorAll('.gw-card').forEach(card=>{
        ...
        card.querySelector('[data-prepare]')?.addEventListener('click',async e=>{const btn=e.currentTarget,hadCopy=!!ta.value.trim(),state=card.querySelector('.gw-status');btn.disabled=true;if(state){state.textContent=hadCopy?'Generando otra opción…':'Preparando copy…';state.className='gw-status ready';}status(hadCopy?'Generando un segundo copy distinto…':'Preparando copy para Google…');try{const d=await call({action:'prepare',id,regenerate:hadCopy});ta.value=d.copy||'';item.google_copy=d.copy||'';item.google_status='ready';if(state){state.textContent='Listo para Google';state.className='gw-status ready';}btn.textContent='Generar otro copy';status(hadCopy?'Nuevo copy generado. Puedes volver a generar otro si quieres.':'Copy de Google listo.');}catch(err){if(state)state.textContent=hadCopy?'Listo para Google':'Sin preparar';status(err.message);}finally{btn.disabled=false;}});
