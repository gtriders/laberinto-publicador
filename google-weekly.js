(() => {
  ...
  function boot(){
    ...
    const mediaUrls=item=>{const fromItems=Array.isArray(item.media_items)?item.media_items.map(x=>typeof x==='string'?x:String(x?.media_url||'')).filter(x=>x.startsWith('https://')):[];const first=String(item.media_url||'');const all=first.startsWith('https://')?[first,...fromItems]:fromItems;return [...new Set(all)];};
    const mediaOf=item=>mediaUrls(item)[0]||'';
    ...
    async function downloadAll(urls,baseName){if(!urls.length)throw new Error('No encontré imágenes para descargar');const files=await Promise.all(urls.map(async(url,i)=>{const r=await fetch(url);if(!r.ok)throw new Error(`No se pudo bajar la imagen ${i+1}`);const blob=await r.blob(),type=(blob.type||'').toLowerCase(),ext=type.includes('png')?'png':type.includes('webp')?'webp':'jpg';return{blob,name:`${baseName}-${String(i+1).padStart(2,'0')}.${ext}`};}));for(let i=0;i<files.length;i++){const file=files[i],href=URL.createObjectURL(file.blob),a=document.createElement('a');a.href=href;a.download=file.name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(href),2000);if(i<files.length-1)await new Promise(resolve=>setTimeout(resolve,180));}return files.length;}
    let items=[];
    function card(item){
      const urls=mediaUrls(item),url=urls[0]||'',state=item.google_status||'unused',copy=item.google_copy||'',published=state==='published',cta=recommend(item),mediaLabel=urls.length===1?'1 imagen':`${urls.length} imágenes`;
      ...
      return `<article class="gw-card" data-id="${item.id}"><div>${url?`<img src="${esc(url)}" alt="${esc(item.title||'Publicación')}"><span class="gw-media-count">${mediaLabel}</span>`:''}</div><div><div class="gw-meta">${esc(brands[item.brand_id]||item.brand_id)} · ${formatDate(item.published_at||item.scheduled_at||item.created_at)}</div><h3>${esc(item.title||'Publicación')}</h3><span class="gw-status ${published?'published':'ready'}">${published?'Publicado en Google':copy?'Listo para Google':'Sin preparar'}</span><textarea class="gw-copy" placeholder="Escribe tu idea, frase o borrador para Google…">${esc(copy)}</textarea><small class="gw-copy-help">Escribe lo que quieres comunicar. La IA lo completará usando el post original solo como contexto.</small>${ctaBlock}<div class="gw-actions">${published?`<button class="btn secondary" data-reset type="button">Volver a pendientes</button>`:`<button class="btn secondary" data-prepare type="button">Completar con IA</button><button class="btn secondary" data-variant type="button">Otra versión</button><button class="btn secondary" data-copy type="button">Copiar copy</button><button class="btn secondary" data-download type="button">${urls.length>1?`Bajar ${urls.length} imágenes`:'Bajar imagen'}</button><button class="btn secondary" data-open type="button">Abrir Google</button><button class="btn primary" data-mark type="button">Marcar publicado</button>`}</div></div></article>`;
    }
    function bind(root){
      root.querySelectorAll('.gw-card').forEach(card=>{
        ...
        ta?.addEventListener('change',async()=>{if(!ta.value.trim())return;try{await call({action:'save_copy',id,copy:ta.value});item.google_copy=ta.value;item.google_status='ready';status('Copy de Google guardado.');}catch(e){status(e.message);}});
        card.querySelector('[data-prepare]')?.addEventListener('click',async e=>{const btn=e.currentTarget,state=card.querySelector('.gw-status'),seed=ta.value.trim();btn.disabled=true;if(state){state.textContent='Completando tu idea…';state.className='gw-status ready';}status(seed?'La IA está completando lo que escribiste…':'Preparando copy desde el post original…');try{const d=await call({action:'prepare',id,seed_text:seed,mode:'complete'});ta.value=d.copy||'';item.google_copy=d.copy||'';item.google_status='ready';if(state){state.textContent='Listo para Google';state.className='gw-status ready';}status(seed?'Copy completado a partir de tu texto.':'Copy de Google listo.');}catch(err){if(state)state.textContent=seed?'Listo para Google':'Sin preparar';status(err.message);}finally{btn.disabled=false;}});
        card.querySelector('[data-variant]')?.addEventListener('click',async e=>{const seed=ta.value.trim();if(!seed){status('Escribe primero una idea o genera el primer copy.');return;}const btn=e.currentTarget,state=card.querySelector('.gw-status');btn.disabled=true;if(state){state.textContent='Generando otra versión…';state.className='gw-status ready';}status('Generando otra versión basada en tu texto…');try{const d=await call({action:'prepare',id,seed_text:seed,mode:'variant'});ta.value=d.copy||'';item.google_copy=d.copy||'';item.google_status='ready';if(state){state.textContent='Listo para Google';state.className='gw-status ready';}status('Nueva versión lista.');}catch(err){if(state)state.textContent='Listo para Google';status(err.message);}finally{btn.disabled=false;}});
        ...
        card.querySelector('[data-download]')?.addEventListener('click',async e=>{const btn=e.currentTarget,urls=mediaUrls(item);btn.disabled=true;try{const total=await downloadAll(urls,`google-${item.brand_id}-${id.slice(0,8)}`);status(total>1?`${total} imágenes descargadas para Google.`:'Imagen lista para subir a Google.');}catch(err){status(err.message);}finally{btn.disabled=false;}});
